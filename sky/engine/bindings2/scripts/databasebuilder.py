...
def _compile_idl_file(build, file_name, import_options):
  try:
    ...
    idl_definition = build.idl_reader.read_idl_definitions(idl_file_fullpath)
    return idl_definition
  except Exception as err:
    print 'ERROR: idl_reader.py: ' + os.path.basename(file_name)
    print err
    print
    print 'Stack Dump:'
...
class Build():
    def __init__(self, provider, interfaces_info_path=None):
        ...
        if interfaces_info_path:
          global interfaces_info
          interfaces_info.update(list(utilities.read_pickle_files([interfaces_info_path]))[0])
        ...
        self.idl_compiler = compiler.IdlCompilerDart(self.output_directory,
                                            attrib_file,
                                            interfaces_info_filename=interfaces_info_path,
                                            only_if_changed=True)
        self.idl_reader = IdlReader(self.idl_compiler.interfaces_info, self.output_directory)
...
class DatabaseBuilder(object):
  def __init__(self, database, interfaces_info_path=None):
    ...
    self.build = Build(None, interfaces_info_path=interfaces_info_path)
  ...
  def _build_signatures_map(self, idl_node_list):
    ...
    for idl_node in idl_node_list:
      ...
      if sig is None:
        continue
      if hasattr(idl_node, 'ext_attrs') and idl_node.ext_attrs.get('PartialInterfaceImplementedAs'):
        continue
      if sig in res:
        op = res[sig]
        ...
        if idl_node.is_fc_suppressed == op.is_fc_suppressed:
          import json
          print json.dumps(op.to_dict(), indent=1, sort_keys=True)
          print json.dumps(idl_node.to_dict(), indent=1, sort_keys=True)
...
#    for interface, import_options in imported_interfaces.iteritems():
  ...
  def _process_idl_file(self, idl_file, import_options):
